...
class DataTableClass
{
    ...
    function memberTable($data){
        ...
        $columns = [
            ...
            ['db' => 'received_at', 'dt' => 6,'formatter' => function($received_at){
                return !empty($received_at) ? date("m/d/Y h:i A",strtotime($received_at)) : "";
            }],
            ['db' => 'id', 'dt' => 7, 'formatter' => function($id, $rowData){
                $element = "";
                if(empty($rowData->received_at)){    
                    $element = "<button type='submit' class='btn btn-sm btn-primary elevation-1 editBtn' data-id='".$id."'><i class='fas fa-edit' aria-hidden='true'></i></button>";   
                }else{
                    $element = "<p style='font-size: 0.9rem !important;' class='text-center font-weight-bold m-0 p-1 rounded-lg elevation-1 border border-success text-success'>RECEIVED</p>";
                }
                
                if(Auth::user()->username == "admin" || Auth::user()->username == "mis"){
                    $element .= "<button type='submit' class='btn btn-sm btn-danger elevation-1 m-2 editShareCapital' data-id='".$id."'><i class='fas fa-sync' aria-hidden='true'></i></button>"; 
                }
